...
export class RAGService {
   ...
   */
  async createKnowledgeBase(data: Omit<KnowledgeBase, 'id' | 'lastUpdated'>): Promise<string> {
    ...
    try {
      ...
      await db.query(
        'INSERT INTO knowledge_bases (id, type, name, description, data, created_at) VALUES ($1, $2, $3, $4, $5, $6)',
        [id, data.type, data.name, data.description, JSON.stringify(knowledgeBase), new Date()]
      );

      ragLogger.info('Knowledge base created', {
        id,
        type: data.type,
        name: data.name
      });
      ...
      ragLogger.error('Failed to create knowledge base', {
   ...
   */
  async addDocument(knowledgeBaseId: string, document: Omit<KnowledgeDocument, 'id' | 'embedding'>): Promise<string> {
    ...
    try {
      ...
      const db = getDatabase();
      await db.query(
        'UPDATE knowledge_bases SET data = $1, updated_at = $2 WHERE id = $3',
        [JSON.stringify(knowledgeBase), new Date(), knowledgeBaseId]
      );

      ragLogger.info('Document added to knowledge base', {
        knowledgeBaseId,
        documentId,
      ...
      ragLogger.error('Failed to add document to knowledge base', {
        error: error instanceof Error ? error.message : String(error),
        knowledgeBaseId,
        title: document.title
      });
   ...
   */
  async searchKnowledgeBase(knowledgeBaseId: string, query: string, limit: number = 5): Promise<KnowledgeDocument[]> {
    try {
      const knowledgeBase = await this.getKnowledgeBase(knowledgeBaseId);
      if (!knowledgeBase) {
        throw new Error(`Knowledge base not found: ${knowledgeBaseId}`);
      ...
      ragLogger.info('Knowledge base searched', {
        knowledgeBaseId,
      ...
      ragLogger.error('Knowledge base search failed', {
        error: error instanceof Error ? error.message : String(error),
        knowledgeBaseId,
        query: query.substring(0, 100)
      });
  ...
  async getKnowledgeBase(id: string): Promise<KnowledgeBase | null> {
    try {
      ...
      return result.rows[0].data as KnowledgeBase;
    } catch (error) {
      ragLogger.error('Failed to get knowledge base', {
        error: error instanceof Error ? error.message : String(error),
        id
      });
      return null;
   ...
   */
  async listKnowledgeBases(type?: string): Promise<KnowledgeBase[]> {
    try {
      const db = getDatabase();
      ...
      return result.rows.map(row => row.data as KnowledgeBase);
    } catch (error) {
      ragLogger.error('Failed to list knowledge bases', {
        error: error instanceof Error ? error.message : String(error),
        type
      });
      return [];
   ...
   */
  async generateContext(knowledgeBaseIds: string[], query: string): Promise<string> {
    let context = '';
    ...
    for (const kbId of knowledgeBaseIds) {
      try {
        ...
        ragLogger.warn('Failed to get context from knowledge base', {
   ...
   */
  async updateDocument(knowledgeBaseId: string, documentId: string, updates: Partial<KnowledgeDocument>): Promise<boolean> {
    try {
      const knowledgeBase = await this.getKnowledgeBase(knowledgeBaseId);
      if (!knowledgeBase) {
        throw new Error(`Knowledge base not found: ${knowledgeBaseId}`);
      ...
      ragLogger.info('Document updated in knowledge base', {
        knowledgeBaseId,
        documentId,
      ...
      ragLogger.error('Failed to update document', {
        error: error instanceof Error ? error.message : String(error),
        knowledgeBaseId,
        documentId
      });
  ...
  async deleteDocument(knowledgeBaseId: string, documentId: string): Promise<boolean> {
    try {
      ...
      ragLogger.info('Document deleted from knowledge base', {
        knowledgeBaseId,
        documentId
      });
      ...
      ragLogger.error('Failed to delete document', {
        error: error instanceof Error ? error.message : String(error),
        knowledgeBaseId,
        documentId
      });
