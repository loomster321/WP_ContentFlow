...
class ContentAgent extends base_agent_1.BaseAgent {
    ...
    anthropicService;
    constructor(config) {
        super('content-agent', 'Content Creation Agent', 'content', [
            'blog-posts',
            'articles',
            'marketing-copy',
    ...
    canHandle(request) {
        ...
        const hasContentKeywords = contentTypes.some(type => prompt.includes(type.replace('-', ' ')) ||
        ...
        const hasContentIndicators = contentIndicators.some(indicator => prompt.includes(indicator));
    ...
    async generateContent(request) {
        ...
        try {
            ...
            logger_1.aiLogger.info('Content agent generated response', {
            ...
            logger_1.aiLogger.error('Content agent generation failed', {
     ...
     */
    calculateSEOScore(content, prompt) {
        let score = 0.5; // Base score
        // Check for keyword presence (extracted from prompt)
        const promptWords = prompt.toLowerCase().split(' ');
        const contentLower = content.toLowerCase();
        // Simple keyword density check
        const keywordMatches = promptWords.filter(word => word.length > 3 && contentLower.includes(word)).length;
        if (keywordMatches > 0) {
            score += 0.2;
        }
        ...
        const hasStructure = sentences.some(sentence => sentence.trim().length > 0 && sentence.trim().length < 50);
        if (hasStructure) {
            score += 0.1;
        }
